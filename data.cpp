...
//EDIT
bool data::editDataPerson(InfoType p)
{
    QSqlDatabase db = QSqlDatabase::database("first");
    QSqlQuery query(db);
    ...
    query.prepare("UPDATE persons SET name = :name, sex = :sex, yearBorn = :yearBorn, yearDead = :yearDead WHERE id = :id");
    query.bindValue(":name", qName);
    query.bindValue(":sex", qSex);
    query.bindValue(":yearBorn", p.birthYear);
    query.bindValue(":yearDead", p.deathYear);
    ...
    if(!query.exec())
    {
        qDebug() << "editPersons error:  " << query.lastError();
    }
    return success;
}

bool data::editDataComputer(CompType c)
{

    QSqlDatabase db = QSqlDatabase::database("first");
    QSqlQuery query(db);

    QString qcompName = QString::fromUtf8(c.compName.c_str());
    QString qtype = QString::fromUtf8(c.type.c_str());

    query.prepare("UPDATE computers SET compName = :compName, yearMade = :yearMade, type = :type, wasBuilt = :wasBuilt WHERE id = :id");
    query.bindValue(":compName", qcompName);
    query.bindValue(":yearMade", c.yearMade);
    query.bindValue(":type", qtype);
